function getBinsEmissionFractions(rtm::RayTracingMeshOptim, wavelength_bands::Vector{G}, temperatures::Vector{G}) where {G}
    ...
    for ((coarse_index, fine_index, wall_index), surface_index) in rtm.surface_mapping
        ...
        for spectral_pos in 1:rtm.n_spectral_bins
            F_0_to_lambda_T = emitFracBlackBody_element_spectrum(wavelength_bands, temperatures[surface_index], spectral_pos)
    ...
    for ((coarse_index, fine_index), volume_index) in rtm.volume_mapping
        F_0_to_lambda_T_prev = 0
        for spectral_pos in 1:rtm.n_spectral_bins
            F_0_to_lambda_T = emitFracBlackBody_element_spectrum(wavelength_bands, temperatures[num_surfaces + volume_index], spectral_pos)
            if spectral_pos == rtm.n_spectral_bins
