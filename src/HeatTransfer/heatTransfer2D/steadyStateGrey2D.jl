...
function get_local_omega(mesh::RayTracingMeshOptim, vol_idx::Int, spectral_bin::Int)
    vol_count = 0
    for coarse_idx in 1:length(mesh.coarse_mesh)
        for (fine_idx, fine_face) in enumerate(mesh.fine_mesh[coarse_idx])
            ...
            if vol_count == vol_idx
                ...
                return local_beta > 0.0 ? local_sigma_s / local_beta : 0.0
            end
        end
    end
    return 0.0
...
function steadyState_optimized_variable!(mesh::RayTracingMeshOptim, F::Matrix{P}; spectral_bin::N=1) where {P<:Real, N<:Integer}
    ...
    for i in 1:n
        for j in 1:n
            ...
            B_ji = if j <= N_surfs
                ...
                get_local_omega(mesh, j - N_surfs, spectral_bin)
            end
            
            B_ij = if i <= N_surfs
                1.0 - ws.epsw[i]
            else
                get_local_omega(mesh, i - N_surfs, spectral_bin)
            end
    ...
    for i in 1:n
        ...
        for k in 1:n
            ...
            B_ki = if k <= N_surfs
                ...
                get_local_omega(mesh, k - N_surfs, spectral_bin)
            end
...
# Updated main memory-optimized steady state function
function steadyState_optimized_variable_matrices!(mesh::RayTracingMeshOptim, F::Matrix{P}; spectral_bin::Int=1) where {P<:Real}
    println("=== Variable Extinction Memory-Optimized Steady State Solver ===")
    ...
    println("Computing temperatures with variable extinction...")
    T = ws.work_vec1  # Reuse work_vec1
    compute_temperatures_variable!(T, j, r, ws, mesh, N_surfs, N_vols, spectral_bin)
    ...
    mesh.energy_error = sum(C_save * j)
...
# Main interface
function steadyStateGrey2D!(mesh::RayTracingMeshOptim, F::Matrix{P};
                        return_matrices::Bool=false, spectral_bin::Int=1) where {P<:Real}
    if !return_matrices
        return steadyState_optimized_variable!(mesh, F; spectral_bin=spectral_bin)
    else
        return steadyState_optimized_variable_matrices!(mesh, F; spectral_bin=spectral_bin)
    end
end