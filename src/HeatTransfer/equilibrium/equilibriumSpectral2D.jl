function equilibriumSpectral2D_full!(rtm::RayTracingDomain2D, F_matrices::Union{Matrix{G}, Vector{Matrix{G}}}; 
    ...
    for bin in 1:rtm.n_spectral_bins
        C, D, M = buildSystemMatrices!(rtm, F_matrices[bin]; spectral_bin=bin)
        push!(D_matrices, D)
        push!(C_matrices, C)
        push!(M_matrices, M)
    end
...
function equilibriumSpectral2D_direct!(rtm::RayTracingDomain2D, F_matrices::Union{Matrix{G}, Vector{Matrix{G}}}) where {G}
    """
    ...
    C, D, M = buildSystemMatrices!(rtm, F_matrices; spectral_bin=1)
