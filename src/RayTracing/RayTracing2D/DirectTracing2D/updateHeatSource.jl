function updateSpectralResults!(domain::RayTracingDomain2D, absorbed_count::Vector{Vector{P}},
    ...
    for ((coarse_index, fine_index), volume_index) in domain.volume_mapping
        sub_face = domain.fine_mesh[coarse_index][fine_index]
...
function writeTemperaturesHeatSourcesDirect!(domain::RayTracingDomain2D)
    ...
    if domain.spectral_mode != :spectral_variable
        ...
        temperatures_calc = zeros(G, length(domain.surface_mapping) + length(domain.volume_mapping))
        for ((coarse_idx, fine_idx, wall_index), surface_idx) in domain.surface_mapping
            sub_face = domain.fine_mesh[coarse_idx][fine_idx]
        ...
        for ((coarse_idx, fine_idx), volume_idx) in domain.volume_mapping
            ...
            if sub_face.T_in_g < -0.1
                ...
                temperatures_calc[length(domain.surface_mapping) + volume_idx] = sub_face.T_g
            else
        ...
        temperatures_init = zeros(G, length(domain.surface_mapping) + length(domain.volume_mapping))
        for ((coarse_idx, fine_idx, wall_index), surface_idx) in domain.surface_mapping
            sub_face = domain.fine_mesh[coarse_idx][fine_idx]
            if sub_face.T_in_w[wall_index] > -0.1
        ...
        for ((coarse_idx, fine_idx), volume_idx) in domain.volume_mapping
            sub_face = domain.fine_mesh[coarse_idx][fine_idx]
            if sub_face.T_in_g > -0.1
                temperatures_init[length(domain.surface_mapping) + volume_idx] = sub_face.T_in_g
                sub_face.T_g = sub_face.T_in_g
            end
        end
