...
@inline function findFaceUniformGrid2D(faces::Vector{PolyVolume2D{G}}, point, 
    ...
    @inbounds for face_idx in grid.cells[cell_i, cell_j]
        if pointInPolygonFast2D(pt, faces[face_idx])
            return face_idx
        end
    end
...
@inline function findFaceWithBboxPrefilter2D(faces::Vector{PolyVolume2D{G}}, point, 
    ...
    @inbounds for i in eachindex(faces)
        ...
        if pointInBbox2D(pt, bboxes[i])
            # Only do expensive point-in-polygon test if bbox test passes
            if pointInPolygonFast2D(pt, faces[i])
                return i
            end
        end
    end
...
@inline function findFace2D(faces::Vector{PolyVolume2D{G}}, point, 
    ...
    if bboxes !== nothing
        return findFaceWithBboxPrefilter2D(faces, pt, bboxes)
    end
...
@inline function pointInBbox2D(point::Point2{T}, bbox::BoundingBox2D{T}) where T
    px, py = point[1], point[2]
    return bbox.min_x <= px <= bbox.max_x && bbox.min_y <= py <= bbox.max_y
end

# Optimized point-in-polygon test
@inline function pointInPolygonFast2D(point::Point2{G}, face::PolyVolume2D{G}) where {G}
