...
"""
function viewFactor3D(POLY_A::Matrix{G}, POLY_B::Matrix{G}) where {G}
    ...
    for p = 1:M # loop through vertices of polygon B
        for i = 1:N  # loop through vertices of polygon A
            ...
            dMin, sOrigin, sHat, lHat, lOrigin, skew = edgePairParameters3D(r_i, r_j, r_p, r_q, almostZero)
            ...
            if skew # if these edges are NOT parallel...
                ...
                sumTerms[i,p] = (cosAlpha*(f3D(s_j, l_q, alpha, cosAlpha, sinAlpha, dMin, almostZero)
                    - f3D(s_i, l_q, alpha, cosAlpha, sinAlpha, dMin, almostZero)
                    - f3D(s_j, l_p, alpha, cosAlpha, sinAlpha, dMin, almostZero)
                    + f3D(s_i, l_p, alpha, cosAlpha, sinAlpha, dMin, almostZero)))
                ...
                skewPairs[i,p] = 0
                sumTerms[i,p] = dot(sHat, lHat)*(fParallel3D(s_j, l_q, dMin, almostZero)
                    - fParallel3D(s_i, l_q, dMin, almostZero) - fParallel3D(s_j, l_p, dMin, almostZero)
                    + fParallel3D(s_i, l_p, dMin, almostZero))
            end
        end
    end
