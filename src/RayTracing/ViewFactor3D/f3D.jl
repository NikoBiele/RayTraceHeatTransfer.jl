...
"""
function f3D(s, l, alpha, cosAlpha, sinAlpha, d, almostZero::G) where G
  ...
  F = ( (0.5*cosAlpha*(s2 + l2) - s*l)*log(s2 + l2 - 2*s*l*cosAlpha + d2) +
       ...
       0.5*(d2/sinAlpha)*(imagLi2_3D((wdim/pdim), alpha, almostZero) +
       imagLi2_3D((pdim/wdim), alpha, almostZero) - 2*imagLi2_3D((wdim - 2*s)/pdim, (pi - alpha), almostZero))  )
