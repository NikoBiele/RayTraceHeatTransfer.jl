...
@inline function find_face_uniform_grid(faces::Vector{PolyVolume2D{G}}, point, 
                                        grid) where {G}
...
@inline function find_face_with_bbox_prefilter(faces::Vector{PolyVolume2D{G}}, point, 
                                              bboxes) where {G}
    # Convert point to the right type if needed
    pt = Point2{G}(point[1], point[2])
...
@inline function find_face_optimized(faces::Vector{PolyVolume2D{G}}, point, 
                                    grid, bboxes) where {G}
    # Convert point to the right type if needed
    pt = Point2{G}(point[1], point[2])
...
@inline function point_in_polygon_fast(point::Point2{G}, face::PolyVolume2D{G}) where {G}
