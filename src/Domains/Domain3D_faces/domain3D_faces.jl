mutable struct Face3D{G}
    ...
    j_w::Union{Nothing, G} # outgoing power [W]
    g_a_w::Union{Nothing, G} # incident absorbed power [W]
    e_w::Union{Nothing, G} # emissive power [W]
    r_w::Union{Nothing, G} # reflected power [W]
    g_w::Union{Nothing, G} # incident power [W]
    i_w::Union{Nothing, G} # total intensity [W*m^(-2)*sr^(-1)]
    q_in_w::G # input source term [W]
    q_w::Union{Nothing, G} # source term [W]
    T_in_w::G # input temperature [K]
    T_w::Union{Nothing, G} # temperature [K]
end
...
function Domain3D_faces(points::Matrix{G}, faces::Matrix{P}, Ndims::P,
                q_in_w::Vector{G}, T_in_w::Vector{G}, epsilon::Vector{G}) where {G, P<:Integer}
    ...
    for (i, face_rows) in enumerate(eachrow(faces))
        if length(face_rows) == 4
            points3d = [Point3{G}(points[face_rows[1],:]), Point3{G}(points[face_rows[2],:]), Point3{G}(points[face_rows[3],:]), Point3{G}(points[face_rows[4],:])]
        else
            points3d = [Point3{G}(points[face_rows[1],:]), Point3{G}(points[face_rows[2],:]), Point3{G}(points[face_rows[3],:])]
        end
    ...
    for i in 1:num_faces
        ...
        for j in 1:num_points
            p1 = Point3{G}(mesh3D[i][1][j][1], mesh3D[i][1][j][2], mesh3D[i][1][j][3])
            p2 = Point3{G}(mesh3D[i][2][j][1], mesh3D[i][2][j][2], mesh3D[i][2][j][3])
            p3 = Point3{G}(mesh3D[i][3][j][1], mesh3D[i][3][j][2], mesh3D[i][3][j][3])
            p4 = Point3{G}(mesh3D[i][4][j][1], mesh3D[i][4][j][2], mesh3D[i][4][j][3])
