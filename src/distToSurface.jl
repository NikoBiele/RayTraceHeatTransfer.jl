function distToSurface(point::SVector{2,Float64}, dir::SVector{2,Float64}, wallPointTop::SVector{2,Float64},
    ...
    u_top = (dot((wallPointTop.-point),topWallNormal))/(dot(dir,topWallNormal))
    u_bottom = (dot((wallPointBottom.-point),bottomWallNormal))/(dot(dir,bottomWallNormal))
    u_left = (dot((wallPointLeft.-point),leftWallNormal))/(dot(dir,leftWallNormal))
    u_right = (dot((wallPointRight.-point),rightWallNormal))/(dot(dir,rightWallNormal))
